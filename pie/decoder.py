...
class AttentionalDecoder(nn.Module):
    ...
    def __init__(self, label_encoder, in_dim, hidden_size, dropout=0.0):
        ...
        self.rnn = nn.GRU(in_dim, hidden_size, dropout=dropout)
    ...
    def forward(self, inp, lengths, enc_outs):
        """
    ...
    def loss(self, inp, lengths, enc_outs, targets):
        logits = self(inp, lengths, enc_outs).view(-1, len(self.label_encoder))
        loss = F.cross_entropy(logits, targets.view(-1), weight=self.nll_weight,
                               size_average=False)
        return loss / targets.ne(self.label_encoder.get_pad()).sum().item()
